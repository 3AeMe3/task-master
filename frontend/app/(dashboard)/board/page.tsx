...
const columns = [
  { id: "PENDIENTE", title: "Pendientes", emptyLabel: "No hay tareas pendientes." },
  { id: "COMPLETADO", title: "Completadas", emptyLabel: "No hay tareas completadas." },
    ...
    <main className="min-h-screen p-10">
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        {columns.map((column) => {
          ...
          return (
            <section key={column.id} className="rounded-2xl bg-white p-4 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-lg font-semibold text-black">{column.title}</h2>
              ...
              <div className="flex min-h-80 flex-col gap-4">
                {columnTasks.length === 0 ? (
                  <div className="flex min-h-52 items-center justify-center rounded-xl border border-dashed border-gray-300 p-6 text-center text-sm text-black/50">
                    {column.emptyLabel}
                  </div>
                ) : (
                  columnTasks.map((task) => (
                    <Card key={task.id}>
                      ...
                      <CardContent className="space-y-3 text-sm text-black/60">
                        <p>{task.description || "Sin descripción"}</p>
                        <div className="flex items-center justify-between">
                          <span>{task.project?.name ?? "Sin proyecto"}</span>
                          <span>
                            {task.dueDate
                              ? dateFormater(task.dueDate)
                              : "Sin fecha"}
                          </span>
                        </div>
                      </CardContent>
